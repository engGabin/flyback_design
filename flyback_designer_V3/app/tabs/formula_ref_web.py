...
FORMULA_HTML_WEB = r"""<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<body>
<h2>1 — Input Power (calc_inputPower)</h2>

<div class="eq">
  \[
  \begin{aligned}
  P_{out,total} &= P_{out1} + P_{out2} + P_{aux} \\
...
<h2>2 — Bulk Capacitor (calc_bulkCapacitance)</h2>

<div class="eq">
  \[
  \begin{aligned}
  V_{ripple} &= V_{in,min}\,\Delta V_{bulk} \\
...
<h2>3 — Preliminary Sizing of the Transformer (calc_preDesign_transformer)</h2>

<div class="eq">
  \[
  \begin{aligned}
  V_{OR} &= \frac{D_{max}\,V_{bulk,min}}{1 - D_{max}} \\
...
<h3>Primary Current Estimations:</h3>

<div class="eq">
  \[
  \begin{aligned}
  I_{p,avg} &= \frac{P_{out,total}}{\eta\, V_{bulk,min}} \\
...
<h3>Secondary Current Estimations:</h3>

<div class="eq">
  \[
  \begin{aligned}
  I_{s,max} &= \frac{2\,I_{out1}}{D_{out}\left(2 - K_{rp}\right)} \\
...
<div class="eq">
  \[
  \begin{aligned}
  A_eA_w = L_p\,\frac{I_{p,max}}{B_{max}}\,k_b\left(\frac{I_{p,rms}}{J_{max}} + \frac{I_{s,rms}}{J_{max}\,\dfrac{N_p}{N_{s1}}}\right)
  \end{aligned}
  \]
</div>
<h2>4 — Transformer (calc_transformer)</h2>

<div class="eq">
  \[
  \begin{aligned}
  N_{P} &= \sqrt{\frac{L_p}{A_l \cdot 10^{-9}}} \\
...
<h2>5 — Final Flyback State (calc_flybackState)</h2>
<h3>Voltages:</h3>

<div class="eq">
  \[
  \begin{aligned}
  V_{OR} &= \frac{N_p}{N_{s1}}\left(V_{out1} + V_F\right) \\
V_{DS,on} &= \frac{V_{bulk,min} + V_{OR}}{1 + \dfrac{V_{bulk,min}\,V_{OR}}{R_{DS,on}\,P_{in}}}
  \end{aligned}
  \]
</div>
...
<div class="eq">
  \[
  \begin{aligned}
  I_{p,avg} &= \frac{P_{out,total}}{V_{bulk,min}\,\eta} \\
I_{p,avg,on} &= \frac{P_{out,total}}{V_{bulk,min}\,\eta\,D_{max}} \\
...
<h3>Duty Cycle and Secondary Current (subscript $k \in \{1,2\}$):</h3>\n
<div class="eq">
  \[
  \begin{aligned}
  D_{out} &= \frac{\left(V_{bulk,min} - V_{DS,on}\right) D_{max}}{V_{OR}} \\
...
<div class="eq">
  \[
  \begin{aligned}
  l_g\ [\mathrm{m}] &= \frac{4\pi\cdot 10^{-7}\,N_p^{2}\,A_e\cdot 10^{-6}}{L_{p,real}} - \frac{l_e\cdot 10^{-3}}{\mu_{core}} \\
l_g\ [\mathrm{mm}] &= l_g\ [\mathrm{m}]\cdot 10^{3} \\
F_{ringing} &= 1 + \frac{l_g\ [\mathrm{m}]}{\sqrt{A_e\cdot 10^{-6}}}\,\ln\!\left(\frac{2g\cdot 10^{-3}}{l_g\ [\mathrm{m}]}\right) \\
B_{max,real} &= \frac{L_{p,real}\, I_{p,max}}{N_p\, A_e\cdot 10^{-6}} \\
...
<h2>6 — Wire Diameter (calc_wire_sections)</h2>

<div class="eq">
  \[
  \begin{aligned}
  \delta\ [\mathrm{cm}] &= \frac{6.62}{\sqrt{f_{sw}}} \\
...
<h3>For every winding $x \in \{p, s1, s2\}$ (theoretical effective section):</h3>\n
<div class="eq">
  \[
  \begin{aligned}
  S_{x,eff} &= \frac{I_{x,rms}}{J_{max}} \\
D_x &= \sqrt{\frac{4\,S_{x,eff}}{\pi}} \\
n_{strands,x} &= \left\lceil \frac{S_{x,eff}}{S_w} \right\rceil
  \end{aligned}
  \]
</div>
<h3>Actual cross-sections based on the selected diameter, for $x \in \{p, s1, s2, aux\}$:</h3>\n
<div class="eq">
  \[
  \begin{aligned}
  S_{x,eff} &= \pi\left(\frac{D_x}{2}\right)^{2} \\
n_{strands,x} &= \left\lceil \frac{S_{x,eff}}{S_w} \right\rceil
  \end{aligned}
  \]
</div>
<h2>7 — Checking the Filling of the Roll-Up Window (check_core_window_fit)</h2>

<div class="eq">
  \[
  \begin{aligned}
  A_{w,used} &= \left(N_p S_{p,eff} + N_{s1} S_{s1,eff} + N_{s2} S_{s2,eff} + N_{aux} S_{aux,eff}\right) k_b \\
\text{fits\_in\_core} &= \left(A_{w,used} \le A_w\right)
  \end{aligned}
  \]
</div>
<h2>8 — Core and Copper Losses (calc_pfe_losses)</h2>

<div class="eq">
  \[
  \begin{aligned}
  \delta_{20} &= 100\sqrt{\frac{\rho_{Cu,20}}{\pi\, f_{sw}\, \mu_0\, \mu_{r,nonmag}}} \\
\delta_{100} &= 100\sqrt{\frac{\rho_{Cu,100}}{\pi\, f_{sw}\, \mu_0\, \mu_{r,nonmag}}}
  \end{aligned}
  \]
</div>
<h3>Diameter to skin depth ratio, for $x \in \{p, s1, s2, aux\}$:</h3>\n
<div class="eq">
  \[
  \begin{aligned}
  Q_x = \frac{D_x}{\delta_{100}}
  \end{aligned}
  \]
</div>
<h3>DC and AC resistances (Dowell coefficients), for $x \in \{p, s1, s2, aux\}$:</h3>\n
<div class="eq">
  \[
  \begin{aligned}
  R_{dc,x} &= \frac{\rho_{Cu,100}\, MT_l\, N_x}{S_{x,eff}} \\
R_{ac,x} &= R_{dc,x}\, K_{ac,x}
  \end{aligned}
  \]
</div>
<h3>Copper losses, for $x \in \{p, s1, s2, aux\}$:</h3>\n
<div class="eq">
  \[
  \begin{aligned}
  P_{Cu,x} &= R_{dc,x}\, I_{x,dc}^{2} + R_{ac,x}\, I_{x,ac}^{2} \\
...
<div class="eq">
  \[
  \begin{aligned}
  \Delta B_{DCM} &= \frac{L_{p,real}\, I_{p,max}}{N_p\, A_e\cdot 10^{-6}} \\
