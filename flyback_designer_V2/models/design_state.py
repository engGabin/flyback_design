...
class DesignState:
    """This class serves solely as a memory (internal database).
        ...
        and the results (what the calculation engine calculates)."""
    ...
    c_bulk_calc: float = 0.0       # calculated bulk capacitor value based on number of hold-up

    c_bulk: float = 23.5e-6        # Input bulk capacitor [F]
    ...
    # ========================================================
    B_max: float = 0.2             # Maximum flux density in the core [T]
    ...
    Ns1_calc: float = 0.0           # secondary turns 1
    ...
    AeAw_calc: float = 0.0          # product of the effective area and the window area
    B_max_calc: float = 0.0         # flux density
...
#     """La Source de Vérité de l'application."""
#     state_changed = pyqtSignal()

#     def __init__(self):
#         super().__init__()
#         self.specs = FlybackSpecs()

#     def notify_update(self):
#         """Déclenche l'alarme pour prévenir l'interface que les données ont changé."""
